...
const autoCompleteJS = new autoComplete({
    ...
    src: async () => {
      try {
        ...
        document
          ...
          .setAttribute("placeholder", "Загрузка...");
        // Fetch External Data Source
        // const source = await fetch(
        //   "https://casino.devcoding.online/generic.json"
        // );
        // const source = await fetch(
        //   "https://tarekraafat.github.io/autoComplete.js/demo/db/generic.json"
        // );
        const source = await fetch("http://localhost:3000/generic.json");
        // const source = await fetch(
        //   "https://casino-anim.devcoding.online/generic.json"
        // );
  ...
  resultsList: {
    element: (list, data) => {
      ...
      if (data.results.length > 0) {
        info.innerHTML = `Показаны <strong>${data.results.length}</strong> из <strong>${data.matches.length}</strong> результатов`;
      } else {
        info.innerHTML = `Найдено <strong>${data.matches.length}</strong> совпадений <strong>"${data.query}"</strong>`;
      }
      list.prepend(info);
    },
    noResults: true,
  ...
  events: {
    input: {
      focus: () => {
        $(".header__innerSearchBox").addClass("active");
        $(".header__innerSearchBox input").addClass("active");
        $(".autoComplete_wrapper").addClass("active");
